...
import { AlertTriangle, BookOpen, Check, RefreshCw, ShieldCheck, X } from 'lucide-vue-next';
...
const busy = computed(() => state.enriching || state.searching);
...
function apply() { emit('apply', metadataStore.applyResolvedMetadata(props.book)); }
...
<template>
  <section class="inspector">
    <header class="inspector-head">
      <div><span>Metadata Inspector</span><h3>Review provider decisions</h3><p>Nothing changes until you accept the proposal.</p></div>
      <button class="icon-button" type="button" @click="close"><X :size="18" /></button>
    </header>

    <div v-if="!state.result" class="inspector-empty">
      <ShieldCheck :size="34" />
      <h4>Compare metadata sources</h4>
      <p>Kindred will query the configured providers and explain which value wins for every field.</p>
      <button class="inspect-button" type="button" :disabled="busy" @click="inspect"><RefreshCw :size="16" :class="{ spin: busy }" /> {{ busy ? 'Inspecting…' : 'Inspect metadata' }}</button>
      <p v-if="state.transportError" class="error">{{ state.transportError.message }}</p>
    </div>

    <template v-else>
      <div class="provider-grid">
        <article v-for="provider in state.providers" :key="provider.providerId" :class="{ failed: provider.error }">
          <div><strong>{{ label(provider.providerId) }}</strong><small v-if="provider.error">{{ provider.error.message }}</small><small v-else>{{ provider.resultCount }} result{{ provider.resultCount === 1 ? '' : 's' }}</small></div>
          <span>{{ provider.durationMs }} ms</span>
        </article>
      </div>

      <div v-if="state.conflicts.length" class="conflicts">
        <h4><AlertTriangle :size="17" /> Conflicts</h4>
        <article v-for="conflict in state.conflicts" :key="conflict.field"><strong>{{ label(conflict.field) }}</strong><span v-for="entry in conflict.values" :key="`${entry.source}-${entry.value}`">{{ label(entry.source) }}: {{ display(entry.value) }}</span></article>
      </div>

      <div class="field-list">
        <article v-for="entry in fields" :key="entry.field">
          <div class="field-title"><strong>{{ label(entry.field) }}</strong><span :class="{ manual: entry.provenance.source === 'manual' }">{{ label(entry.provenance.source || 'unknown') }} · {{ Math.round((entry.provenance.confidence || 0) * 100) }}%</span></div>
          <p>{{ display(entry.value) }}</p>
          <small>{{ entry.decision?.reason || 'resolved metadata value' }}</small>
        </article>
      </div>

      <div v-if="state.coverCandidates.length" class="covers">
        <h4>Cover candidates</h4>
        <div><button v-for="cover in state.coverCandidates.slice(0, 6)" :key="cover.url" type="button" :class="{ selected: state.selectedCover?.url === cover.url }" @click="metadataStore.selectCover(cover)"><img :src="cover.url" alt="Book cover candidate" /><span>{{ label(cover.source) }}</span></button></div>
      </div>
