...
function applyInspector(proposal) {
  const resolved = proposal?.metadata || proposal || {};
  selected.value = {
    ...selected.value,
    ...resolved,
    coverUrl: resolved.cover?.url || resolved.coverUrl || selected.value.coverUrl,
    metadataSource: {
      ...(selected.value.metadataSource || {}),
      ...(proposal?.metadataProvenance || {}),
    },
  };
  inspectorOpen.value = false;
  metadataStore.clear();
...
<template>
  <div class="shell">
    <aside class="sidebar">
      ...
      <div class="library-picker"><label>Library</label><select v-model="activeLibraryId" @change="openLibrary(activeLibraryId)"><option v-for="library in libraries" :key="library.id" :value="library.id">{{ library.name }} · {{ library.itemCount }}</option></select></div>
      <nav><button v-for="type in types" :key="type.id" :class="{ 'nav-active': activeType === type.id }" @click="activeType = type.id"><component :is="type.icon" :size="18" /> {{ type.label }} <span>{{ counts[type.id] || 0 }}</span></button><button><Tags :size="18" /> Tags <span>{{ tagsCount }}</span></button></nav>
      <div class="device" :class="{ online: device.connected || activeLibrary }"><div class="device-icon"><Cable v-if="device.connected" :size="20"/><HardDrive v-else :size="20"/></div><div><strong>{{ activeLibrary?.name || 'No saved library' }}</strong><span>{{ device.connected ? `Connected · ${device.objectCount || 0} objects` : (activeLibrary ? `Offline · ${activeLibrary.itemCount || items.length} saved items` : 'Scan a Kindle once') }}</span></div></div>
    </aside>
    ...
    <main>
      ...
      <section v-else class="grid"><button v-for="item in filtered" :key="item.id" class="book-card" @click="choose(item)"><div class="cover" :style="item.coverUrl ? { backgroundImage: `url(${item.coverUrl})` } : {}"><BookOpen v-if="!item.coverUrl" :size="28"/><span>{{ item.format }}</span></div><div class="book-info"><h3>{{ item.title }}</h3><p>{{ item.authors?.join(', ') || (item.asin ? `ASIN ${item.asin}` : 'Metadata not extracted yet') }}</p><div class="chips"><span v-for="tag in (item.tags || []).slice(0, 2)" :key="tag">{{ tag }}</span><small>{{ formatBytes(item.size) }}</small></div></div></button></section>
    </main>

    <div v-if="reviewingImport" class="scrim import-scrim"><section class="import-review"><header class="drawer-header"><div><span>Review before transfer</span><h2>Add books to Kindle</h2><p>Embedded metadata was read first. Free providers only filled missing fields.</p></div><button class="icon" @click="cancelBookImport"><X/></button></header><div class="import-list"><article v-for="draft in importDrafts" :key="draft.clientId" class="import-book"><div class="import-cover" :style="draft.coverUrl ? { backgroundImage: `url(${draft.coverUrl})` } : {}"><BookOpen v-if="!draft.coverUrl" :size="28"/><span>{{ draft.format }}</span></div><div class="import-fields"><small>{{ draft.fileName }} · {{ formatBytes(draft.size) }}</small><label>Title<input v-model="draft.title" /></label><label>Authors<input :value="draft.authors?.join(', ')" @input="draft.authors = $event.target.value.split(',').map(x => x.trim()).filter(Boolean)" /></label><div class="two"><label>Series<input v-model="draft.series" /></label><label>Number<input v-model.number="draft.seriesIndex" type="number" step="0.1" /></label></div><div class="two"><label>ISBN<input v-model="draft.isbn" /></label><label>Language<input v-model="draft.language" /></label></div><label>Publisher<input v-model="draft.publisher" /></label><label>Tags<input :value="draft.tags?.join(', ')" @input="draft.tags = $event.target.value.split(',').map(x => x.trim()).filter(Boolean)" /></label><label>Description<textarea v-model="draft.description" rows="3" /></label><p v-if="draft.inspectionWarning" class="import-warning">{{ draft.inspectionWarning }}</p></div></article></div><footer class="drawer-footer"><button class="secondary" @click="cancelBookImport">Cancel</button><button class="primary" :disabled="addingBooks" @click="sendReviewedBooks"><UploadCloud :size="17"/> {{ addingBooks ? 'Sending…' : `Send ${importDrafts.length} to Kindle` }}</button></footer></section></div>

    <div v-if="selected" class="scrim" @click.self="closeEditor"><section class="drawer"><header class="drawer-header"><div><span>{{ selected.type }} · {{ selected.format }} · {{ selected.fileName }}</span><h2>Edit metadata</h2></div><button class="icon" @click="closeEditor"><X/></button></header><div class="drawer-body"><div class="editor"><div class="cover-column"><div class="cover-large" :style="selected.coverUrl ? { backgroundImage: `url(${selected.coverUrl})` } : {}"><BookOpen v-if="!selected.coverUrl" :size="42"/></div><button class="secondary" :disabled="metadataLoading" @click="refreshSelectedMetadata"><RefreshCw :size="15" :class="{ spin: metadataLoading }"/> {{ metadataLoading ? 'Matching…' : 'Auto-match metadata' }}</button><button class="secondary" @click="openInspector"><Sparkles :size="15"/> Inspect provider decisions</button><small>Inspect before accepting. Saving remains a separate explicit action.</small></div><div class="fields"><label>Type<select v-model="selected.type"><option v-for="type in types" :key="type.id" :value="type.id">{{ type.label }}</option></select></label><label>Title <small>{{ sourceLabel('title') }}</small><input v-model="selected.title" /></label><label>Authors <small>{{ sourceLabel('authors') }}</small><input :value="selected.authors?.join(', ')" @input="selected.authors = $event.target.value.split(',').map(x => x.trim()).filter(Boolean)" /></label><div class="two"><label>Series<input v-model="selected.series" /></label><label>Number<input v-model.number="selected.seriesIndex" type="number" step="0.1" /></label></div><div class="two"><label>ISBN<input v-model="selected.isbn" /></label><label>ASIN <small>{{ sourceLabel('asin') }}</small><input v-model="selected.asin" /></label></div><div class="two"><label>Language<input v-model="selected.language" /></label><label>Publisher<input v-model="selected.publisher" /></label></div><label>Description<textarea v-model="selected.description" rows="5" /></label><label>Tags<div class="tag-editor"><span v-for="tag in selected.tags" :key="tag">{{ tag }} <button @click="removeTag(tag)">×</button></span><input v-model="tagInput" @keydown.enter.prevent="addTag" placeholder="Add tag…" /></div></label></div></div><MetadataInspector v-if="inspectorOpen" :book="selected" @apply="applyInspector" @close="closeInspector" /></div><footer class="drawer-footer"><button class="secondary" @click="closeEditor">Cancel</button><button class="primary" @click="save"><Check :size="17"/> {{ saving ? 'Saving…' : 'Save changes' }}</button></footer></section></div>
  </div>
</template>
